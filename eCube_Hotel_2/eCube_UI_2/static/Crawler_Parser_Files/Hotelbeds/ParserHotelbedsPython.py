...
from Parsing.scripts.Hotelbeds import ParserConfigHotelbedsPython
...
ParserBase.CONFIG_FILE = ParserConfigHotelbedsPython
...
class RoomType(ParserBase):
    ...
    def parsed__room_types(self):
        ...
        for container in containers:
            roomtype = str(container.xpath(ParserConfigHotelbedsPython.RoomType__type)[0]).strip()
            room_code = str(container.xpath(ParserBase.CONFIG_FILE.Hotel__roomCode)[0]).strip()
            priceblocks = container.xpath(ParserConfigHotelbedsPython.RoomType__priceblocks)
            for priceblock in priceblocks:
                price = priceblock.xpath(ParserConfigHotelbedsPython.RoomType__price)[0]
                # try:
                #     promotionDesc = priceblock.xpath(ParserConfigHotelbedsPython.RoomType__promotion)[0]
                #     promocode = priceblock.xpath(ParserConfigHotelbedsPython.RoomType__promotioncode)[0]
                #     promotion ='Y'
                #
                # except:
                #     promotionDesc=''
                ...
                boardtype = str(priceblock.xpath(ParserConfigHotelbedsPython.RoomType__boardType)[0]).strip()
                boardtype = self.__class__.get_board_code(boardtype)

                roomtypes.append({"rcode":"0",'breakfast': 0,"type": roomtype,"contractName": contractName,"Opaquerate":OpaqueRate,"roomAvailability": int(room_availability), "board_code": boardtype, "price": price,"commision": float(commission) if commission!=[] else "0","selling_price_mandatory":"N",
                                "promotion": isPromotion, "promotionDescription": promoDesc.strip() if isPromotion else "", 'availability':'Available', 'currency':currency,"paymentOption": "Y" if directPayment == "AT_HOTEL" else "N","daily_price": round(float(price) / no_of_days, 2) if price!="" else "",
                                "costCurrency": costCurrency,"tax_included": costTaxpercent,"included1": strPercentageinctax,"tax_not_included": costTaxpercentnotinc,"classification": classification,
                                "not_included": strPercentagenotinctax,"tax$included": strTaxAmount1,"currency_inlcuded": str(strFixedTaxCurr1),"included2": strFixedIncludedTax1,
                                "tax$not_included": strTaxAmount2,"currency_not_included": str(strFixedTaxCurr2),"not_included2": strFixedNotIncludedTax1,"Integration": Integration,"Roomcharval":Roomcharval,
                                "netPrice": float(netPrice) if netPrice!="" else "","sellingPrice": float(sellingPrice) if sellingPrice!="" else "","cost": cost,"costCurrency": costCurrency})
        return roomtypes

...
function Seach_match_unmatch() {
    ...
    var hotels_length =  $("#drp_hotels").val().length;

    if (hotels_length > 500){
        var selected_hotel_type = [];
        var selected_all = 1;
    }else{
...
function setData(data) {
    ...
    var is_match = '';
        if (item[25] != null && item[25].toString().toUpperCase() == 'MATCHED') {
            ...
            sec_htl_link = '<a  data-toggle="modal" data-target="#myModal"  onclick="Bind_secondry_htls_details(' + item[0] + ',' + item[17] + ');" class="select_hotel_click"> Select hotel </a>'
            chk_bx = '';
            read_only = 'disabled';
            is_match = 'false';
        }
...
function Save_match_unmatch() {
    ...
    $(table.fnGetNodes()).each(function (i, item) {
        ...
        if (($(item).find("td:eq(5)").find('p').text().trim() != '' && $(item).find("td:eq(5)").find('p').text().trim().toString() == 'true') || ($(item).find("td:eq(4)").find('p').text().trim() != '')) {
            row_count = row_count + 1;
            if ($(item).find("td:eq(5)").find('p').text().trim().toString() == 'true') {
                unmatch = unmatch + act_id.toString() + ',';
            }
            if ($(item).find("td:eq(4)").find('p').text().trim()!='') {
                var prim_id = $(item).find("td:eq(0)").find('p').text().trim();
                primary_htls_id = primary_htls_id + prim_id + ',';
                sec_id = sec_id + $(item).find("td:eq(4)").find('p').text().trim() + ',';
            }
        }
...
function Bind_secondry_htls_details(rowId, supplierId) {
    rowId = 'tr_grid_match_' + rowId;
    $('#hdn_primary_grid_id').val('');
    $('#hdn_primary_grid_id').val(rowId);
    var city_type = $('#drp_cities').val();
    var city_name =$("#drp_cities option:selected").text();
    var sup_name_popup ='';
    var selected_sup_type = $("#drpCompt option:selected");
    var sup_type = "";
    selected_sup_type.each(function () {
        sup_type += $(this).val() + ",";
        sup_name_popup  += $(this).text() + ",";
    });
    sup_name_popup = sup_name_popup.replace(/,\s*$/, "").trim();
    $('#spn_city_popup').text(city_name);
    $('#spn_sup_popup').text(sup_name_popup);
    sup_type = sup_type.replace(/,\s*$/, "").trim();
    if (city_type != '' && sup_type != '') {
        var model = {
            city_id: city_type,
            secondry_sup_id: supplierId,
        };

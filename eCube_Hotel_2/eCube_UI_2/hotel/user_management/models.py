...
class UserToken(models.Model):
    ...
    token = models.CharField(db_column='token', unique=True, max_length=128)

    class Meta:        
        managed = False
        db_table = 'tbl_UserToken'
...
class UserMenuAccessMapping(UserMenuAccessMappingBase):
    user_id = models.ForeignKey(UserMaster,db_column='user_id',related_name='user_userid', on_delete=models.CASCADE)
    createdby = models.ForeignKey(UserMaster,db_column='createdby',related_name='createdby_userid', on_delete=models.CASCADE)
...
class PointOfSale(PointOfSaleBase):
    ...
    country = models.ForeignKey(CountryMaster, on_delete=models.DO_NOTHING, related_name='point_of_sales')

...
class HRSHotel(HotelHandler):
    ...
    def _get_html(self):
        ...
        """
        return {'landingPage': '', 'hotelHtml': self._html}
    ...
    def _get_room_types(self):
        ...
        """
        pageHtml = html.fromstring(self._html['html_element'])
        room_types = pageHtml.xpath(self.CONFIG_FILE.roomDetailsInContainersXpath)
        for i, j in enumerate(room_types):
            room_str = etree.tostring(j).decode('utf-8')
            room_types[i] = room_str
        price_htmls = pageHtml.xpath(self.CONFIG_FILE.pricesInContainersXpath)
        for i, j in enumerate(price_htmls):
            price_html_str = etree.tostring(j).decode('utf-8')
            price_htmls[i] = price_html_str

        promotions = pageHtml.xpath(self.CONFIG_FILE.roomPromotion)
        for i, j in enumerate(promotions):
            promotions_str = etree.tostring(j).decode('utf-8')
            promotions[i] = promotions_str

        boardtypes = pageHtml.xpath(self.CONFIG_FILE.roomBoardType)
        for i, j in enumerate(boardtypes):
            boardtypes_str = etree.tostring(j).decode('utf-8')
            boardtypes[i] = boardtypes_str
        room_dict = {
            ...
            'roomTypeHTML': room_types,
            'priceHTML': price_htmls,
            'promotionHTML': promotions,
            'boardTypeHTML': boardtypes,
        }
        return room_dict
...
class HRSLandingPage(HotelLandingPageHandler):
    ...
    def _hotels(self):
        ...
        """
        headers = copy.deepcopy(HRSLandingPage.HEADERS)
        headers["Host"] = "osc.hrs.com"
        headers['Referer'] = "https://www.hrs.com/web3/searchWeb1000.do?activity=showHotellistWithPromotion&showOverlay=true&clientId={0}--&cid={1}".format(
                                                self.client_id,
                                                self.cid
                                            )

        if hasattr(headers, 'Cookie'):
            # print('deleting cookie')
            delattr(headers, 'Cookie')
    ...
    def _set_cookie(self, resp_object):
        ...
        if resp_object is not None:
            # print('reading cookie from resp_object')
            # print(resp_object.headers)
    ...
    def _process_homepage(self):
        ...
        :return: None
        """
        # print('HRSLandingPage.HOST')
        # print(HRSLandingPage.HOST)
        # print('self._get_headers')
        # print(self._get_headers)
        # print('self.proxy.to_log()')
        # print(self.proxy.to_log('http'))
        process_homepage_header = {
            'Accept': 'text/html, application/xhtml+xml, */*',
            'Accept-Language': 'en-US,en;q=0.5',
            'Content-Type': 'application/x-www-form-urlencoded',
            'Host': 'www.hrs.com',
        ...
        res_html, res_obj, driver_obj = self.get_request(HRSLandingPage.HOST, process_homepage_header, proxy=self.proxy)
        ...
        self.cid = res_html[cid_idx:res_html.find('&', cid_idx + 1)].split('=')[1]
        ...
        self.client_id = res_html[client_idx:res_html.find('-', client_idx)].split('=')[1]

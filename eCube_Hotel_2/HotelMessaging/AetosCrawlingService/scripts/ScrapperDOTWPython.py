...
class DOTWLandingPage(HotelLandingPageHandler):
    ...
    def _hotels(self):
        ...
        if self.city.upper() == 'Kuala Lumpur'.upper() or self.city.upper() == 'Istanbul'.upper():
    ...
    def _process_homepage(self):
        ...
        for each in self._country_list:
            if each['text'].upper() == self.pos.upper():
...
def crawl_hotels(consumer_data, redelivered):
    ...
    # with open('dotw_crawled_data_%s.json' % file_distincter, 'w+') as file_obj:

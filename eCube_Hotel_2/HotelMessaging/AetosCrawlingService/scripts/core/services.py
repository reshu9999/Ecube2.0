...
from resources.connections import MongoBaseHandler
from scripts.core.utils import get_nohodo_proxy, get_user_agent, hotel_data_save_checker
...
class API(object):
    ...
    def GetProxy(cls, domain, country):
        ...
        # domain = 'Travel'
        url = "http://{0}:{1}/api/v1/getProxy?domain={2}&country=".format(
            cls.PROXYDETAILS['url'], cls.PROXYDETAILS['port'], domain
        )
    ...
    def GetCityMatchingZone(cls):
        ...
        city_data = requests.get(url, verify=False)
    ...
    @classmethod
    def SaveCachePage(cls, filename, html):
        url = 'http://192.168.8.53:8002/api/save_cache_page'
        post_data = {'filename': filename, 'filecontent': html}
        response = requests.post(url, data=json.dumps(post_data))
        return response.text if 'savepage' in response.text else None
...
class MongoHandler(MongoBaseHandler):
    ...
    def mark_hotels_to_be_parsed(self, crawled_data):

...
from Queues.ScraperQueue import travel_republic_config
...
from Queues.ScraperQueue.resources.travel_republic_travellers import travellers_data_dict, city_multi_code, city_mapping, currency
...
class TravelRepublicHomePage():
    ...
    def _get_post_params(cls, query_name, *args, **kwargs):
        ...
        post_data_dict['hotelSearchSubType'] = 1
...
class TravelRepublicSupplierDetail(object):
    ...
    def get_hotel_room_detail(cls, room_row_attribs):
        ...
        res_html, res_obj, _ = cls.OBJ.get_request(room_url, headers=var_headers,
                                                  proxy=cls.OBJ.proxy)
...
def crawl_hotels(consumer_data, redelivered):
    ...
    # try:
    #     crawled_data = TravelRepublicLandingPage(consumer_data).crawl_hotels()
    # except Exception as e:
    #     from pdb import set_trace; set_trace()
    #     copy_data = consumer_data.copy()
    #     copy_data.pop('RequestInputs')
    #     crawled_data = {'meta': copy_data, 'hotels': list()}

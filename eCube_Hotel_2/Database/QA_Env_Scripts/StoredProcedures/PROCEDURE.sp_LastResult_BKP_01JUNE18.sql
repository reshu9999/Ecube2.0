...
CREATE PROCEDURE `sp_LastResult_BKP_01JUNE18`(            
  p_intBatchId  int,        
...
/*
	UPDATE TEMP_BCD SET Hotel = ISNULL(            
		(            
		SELECT  IFNULL(MAX(nvcrHotelName), TEMP_BCD.Supplier)            
		FROM Hotels MH INNER JOIN  HotelRelation HR              
		ON   MH.intHotelId = HR.intHotelId             
		WHERE  (intHotelRelationComHotelId =  TEMP_BCD.intMSTHotelId OR HR.intHotelId =  TEMP_BCD.int HotelsId)            
		),TEMP_BCD.Supplier)              
		WHERE EXISTS            
		(            
			SELECT  intHotelRelationID             
			FROM   HotelRelation              
			WHERE  intHotelRelationComHotelId = TEMP_BCD.intMSTHotelId
		);            
	 ...
	 UPDATE TEMP_BCD SET Hotel = ISNULL(            
		 (            
		  ...
		  ON   MH.intHotelId = HR.intHotelId             
		  WHERE  (HR.intHotelId =  TEMP_BCD.int HotelsId)            
		  ),TEMP_BCD.Supplier)
		 WHERE NOT EXISTS            
		 (             
		  SELECT  intHotelRelationID             
		...
		UPDATE TEMP_BCD A12 
		left outer join (
			Select  HR.intHotelId intHotelId, intHotelRelationComHotelId,bitIsManualUnmatch 
			From   HotelRelation HR  
			where bitIsManualUnmatch = 0  
			order by intHotelRelationId desc
		) rel
		on ( rel.intHotelRelationComHotelId = A12.int HotelsId )  
        SET `HotelId` = COALESCE(rel.intHotelId, (-1 * A12.int HotelsId)) 
...
*/	          
		...
		*/
		...
		On H.intHotelId = T.HotelId Set   T.HotelCode = H.nvcrWebSiteHotelId 
			Where T.intSupplierId <>  1      
		  
		 Update   Hotels H Inner Join TEMP_BCD T
		On H.Unq_HotelID = T.HotelId Set   T.HotelCode = H.nvcrWebSiteHotelId 
			Where T.intSupplierId <> 1       

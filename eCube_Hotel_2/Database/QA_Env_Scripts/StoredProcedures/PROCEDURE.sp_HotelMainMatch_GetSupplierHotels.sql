...
/* No need of this we will fetch only match records
Delete From TEMP Where HotelRelationId in 
(
		Select HotelRelationId From 
		(
					   
			Select ROW_NUMBER() Over(Partition BY T1.hotelID, T1.SupplierId Order By T1.MatchUnMatchDate desc) RowNo ,
			T2.*     From #TEMP T1 Inner Join #TEMP T2 
			On T1.HotelId = T2.HotelId 
            And T1.MatchDate != T2.UnmatchDate 
            And T1.SupplierId = T2.SupplierId 
            And T1.ComSupplierid = T2.ComSupplierid
			And T1.IsManualUnmatch = 0 
            And T2.IsManualUnmatch = 1									
...
-- Added by Bhushan Gaud End */
	...
	-- Select  * From Temp_MatchingStatus;

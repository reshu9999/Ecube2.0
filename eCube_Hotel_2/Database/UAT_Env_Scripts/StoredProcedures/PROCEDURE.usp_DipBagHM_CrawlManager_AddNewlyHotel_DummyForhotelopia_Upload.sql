...
******************************************************************************************************/
        ...
        ,	HotelPostCode,	h.CompetitorId,	HotelMatchStatus,	HotelDescription,	CreatedBy ,	isProceesed,	CreatedDate
		...
		from Hotels h inner join 
				(select Id as CompetitorId from tbl_Competitor 
				where Id in (v_Hotelbeds,v_Bedsonline,v_TUI,v_OpaqueRetailPrice,v_BOLCOM,v_BOLNET,v_HBDNET,v_BOLSG,v_BOLMY,v_BOLID,v_BOLTH) 
                ) s -- 6,25,37,46,70,71,72,73,74,75,76)--PMS# 32387
				on h.CompetitorId = s.CompetitorId 
		...
		,	CrawledHotelAddress,	CrawledHotelStar,	ZoneName,	CrawledZoneName
		from Hotels 
		where CompetitorId = v_hotelopia -- ,@Bedsonline,@TUI,@OpaqueRetailPrice,@BOLCOM,@BOLNET,@HBDNET,@BOLSG,@BOLMY,@BOLID,@BOLTH)--6,25,37,46,70,71,72,73,74,75,76)--PMS# 32387
		...
		,	CrawledHotelAddress,	CrawledHotelStar,	ZoneName,	CrawledZoneName
		from Hotels 
		where CompetitorId = v_hotelopia -- ,@TUI,@OpaqueRetailPrice,@BOLCOM,@BOLNET,@HBDNET,@BOLSG,@BOLMY,@BOLID,@BOLTH)--6,25,37,46,70,71,72,73,74,75,76)--PMS# 32387
		and WebSiteHotelId is not null;
...
/*!50003 DROP PROCEDURE IF EXISTS `USP_SendMail_hoteldeletion_task` */;

...
CREATE PROCEDURE `sp_LastResult`(            
  p_intBatchId  int,        
...
BEGIN                  
	...
	(    IntID bigint Primary Key unique key auto_increment,           
	...
	Select bcrDt.* From BatchCrawlData bcrDt inner join Temp_SupplierID t 
		on bcrDt.IntsupplierID = t.intSupplierId
	...
	SELECT  nvcrSupplierIDExcludeSavePageUrl INTO v_nvcrSupplierIDExcludeSavePageUrl 
		FROM MDM_Batches 
	WHERE intDipBagDynamicId=p_intDipBagDynId ORDER BY intMDM_BatchesId DESC Limit 1;
	...
	SELECT          
        ...
        (select Max(roomtypematch) from vw_HotelStandardization where roomtype like   CONCAT(bcrDt.NVCRROOMTYPE ,'%') ) `RoomCode`, -- '' `RoomCode`,
		bcrDt.nvcrHotelName  Supplier,            
		'' as `CancellationPolicy`,
		ifnull((select StarRatingCode from StarRatings where StarRating=Substring(bcrDt.nvcrhotelstar,PatIndex('/',bcrDt.nvcrhotelstar)+1,char_length(rtrim(bcrDt.nvcrhotelstar))-PatIndex('/',bcrDt.nvcrhotelstar)+1)) ,'PENDI') `StarRating` , -- "" `StarRating` ,
	...
	select  count(1) into v_adhocbatchcount from AdhocLeadTimeLatest where `Batch_Name` = v_nvcrBatchName;
...
*/		
	...
	*/
		...
		on ( rel.HotelRelationComHotelId = A12.intMSTHotelId )  
	...
	if exists(select * from TEMP_BCD where Company= 'Global Market H1' Limit 1) Then 
        ...
        Drop temporary table if exists TEMP_BCD_GH;
        Create temporary table TEMP_BCD_GH
        As
...
Set @rn1 = 1;
		...
		Set @Supplier = '';
		 ...
		 Delete T From TEMP_BCD T Where IntID in 
		 (
			Select B.IntID From 
            (
				Select 
				...
				From 
                (
					SELECT  Country, city, Supplier, HotelId, IntID
						FROM TEMP_BCD_GH P 
		...
		(
			...
			(
				...
				@rn1 := 
					...
					,  @rn1+1) As Rank, A.HotelId, A.IntID, 

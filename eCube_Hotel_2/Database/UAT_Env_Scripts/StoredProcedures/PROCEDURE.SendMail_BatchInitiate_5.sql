...
BEGIN 
	...
	DECLARE v_subject longtext  DEFAULT 
									(Select CASE WHEN p_vcrStatus =v_Start   THEN   CONCAT('Hotelbeds MDM_5 Â– Report generation for batch ' , v_BatchName ,  '  "INITIATED"')
												 WHEN p_vcrStatus =v_Fail    THEN   CONCAT('Hotelbeds MDM_5 Â– Report generation for batch ' , v_BatchName ,  ' "FAILED"')
												 WHEN p_vcrStatus =v_success THEN   CONCAT('Hotelbeds MDM_5 Â– Report generation for batch ' , v_BatchName ,  ' "COMPLETED"') 
												 WHEN p_vcrStatus =v_NoData  THEN	  CONCAT('Hotelbeds MDM_5 Â– Report generation for batch ', v_BatchName  ,' "FAILED"')
												 WHEN p_vcrStatus =v_Error   THEN	  CONCAT('Hotelbeds MDM_5 Â– Report generation for batch', v_BatchName  ,' "FAILED"')	
												 WHEN p_vcrStatus =v_ManualUpload THEN 'Hotelbeds MDM_5 Â– Report uploaded in MDM Database'
												 WHEN p_vcrStatus = v_leadtime THEN CONCAT('Hotelbeds MDM_5 Â– Alert balnk values for LeadTime column  ', v_BatchName  ,' Batch')	-- -added on 22-03-2016 for balnk columns
												 END );							
...
<div style="font-family: Calibri;font-size: 12pt;">
  <p>
        ...
        (Select Case WHEN p_vcrStatus = v_Start THEN 'Report generation for the following batch has been initiated in MDM -'
                     ...
                     WHEN p_vcrStatus = v_NoData THEN 'Report could not be generated for the following batch as batch didnÂ’t have any harvested data -'
  ...
  ',
  -- -(Select ISNULL( Case WHEN @vcrStatus = @success THEN  'Please visit following link to access the Report Â– <br/>

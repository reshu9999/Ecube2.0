...
CREATE PROCEDURE `sp_GetReportUploadUpdate`(
	p_RequestId INT,
	p_RequestRunId INT,
...
BEGIN
	...
	CREATE Temporary  TABLE tmpReportUploadUpdate(intBatchId INT,nvcrBatchName VARCHAR(500),nvcrComment VARCHAR(500), 
 ...
 -- done upto
   ...
   -- BElow insert have problem atart--
   -- 'Uniquecode is not nulll/blank '
 
	INSERT INTO tmpReportUploadUpdate(intBatchId,nvcrBatchName,nvcrComment, IntCheckListID)
	Select  p_RequestId,v_nvcrBatchName,'Test', 241  
		 from LastResultQACheck BCD Where intDiPBagDynamicId = p_RequestRunId                              
		  And ifnull(BCD.Uniquecode,'')!=''
        Limit 1;
        
 
   -- BElow insert have problem end */
	...
	Select  p_RequestId,v_nvcrBatchName,'Currency is null/blank ', 1 IntStatus, 128  
		...
		-- below insert HAVE problem  starts
		/*
	INSERT INTO tmpReportUploadUpdate(intBatchId,nvcrBatchName,nvcrComment, IntStatus, IntCheckListID)                       
...
/*
    ...
    */
	...
	/* do not open
	Select Distinct Concat(Ifnull(intBatchDetailNoOfAdult,'')  , ' Adt'), Concat(Ifnull(tintBatchChild,'') , ' Chd') 
		Into v_NvcrAdlt, v_NvcrChild 
	 From tbl_hotelrequestinputdetails BD 
    ...
    /*
	INSERT INTO tmpReportUploadUpdate(intBatchId,nvcrBatchName,nvcrComment, IntCheckListID)
	Select Distinct p_RequestId,v_nvcrBatchName,CONCAT('Competitor does not exist in batch.' , nvcrDisplaySupplierName), 169  
		 from LastResultQACheck BCD 
		   Right Join 
		(Select BS.intSupplierId, MS.nvcrDisplaySupplierName
			FromBatchSupplier BS  inner join tbl_Competitor MS 
				On BS.intSupplierID =MS.intSupplierId
			Where BS.intBatchId = p_RequestId 	
			) BD 
		On BCD.Company = BD.nvcrDisplaySupplierName  
		And intRequestRunDetailId = p_RequestRunId 
	...
	INSERT INTO tmpReportUploadUpdate(intBatchId,nvcrBatchName,nvcrComment, IntCheckListID)	
	Select  p_RequestId,v_nvcrBatchName,CONCAT('Missing Date: ' , Date_format(sdtmBatchDateCrawl, 106)), 170  
		 from LastResultQACheck BCD 
		   Right Join 
	(Select sdtmBatchDateCrawl, intBatchId From BatchDateDetail  Where intBatchId = p_RequestId) BD
		On Cast(BCD.Dates As Date)= Cast(BD.sdtmBatchDateCrawl As Date)
		And intRequestRunDetailId = p_RequestRunId 
		Where  BCD.Dates Is Null And 
		BD.intBatchId = p_RequestId;
	...
	INSERT INTO tmpReportUploadUpdate(intBatchId,nvcrBatchName,nvcrComment, IntCheckListID)	
	Select Limit 1; p_RequestId,v_nvcrBatchName,'Night does not exist in batch/Incorrect nights selected.', 171  
		 from LastResultQACheck BCD Right Join 
		( 
			Select Distinct intBatchId, sdtmBatchDateCrawl, bitBatchDate1Night Nights FromBatchDateDetail    Where intBatchId = p_RequestId And bitBatchDate1Night = 1
			Union All
			Select Distinct intBatchId, sdtmBatchDateCrawl, 3 Nights FromBatchDateDetail    Where intBatchId = p_RequestId And bitBatchDate3Night = 1
			Union All
			Select Distinct intBatchId, sdtmBatchDateCrawl, 7 Nights FromBatchDateDetail    Where intBatchId = p_RequestId And bitBatchDate7Night = 1
			Union All
			Select Distinct intBatchId, sdtmBatchDateCrawl, sintBatchDateOptionalNight Nights FromBatchDateDetail    Where intBatchId = p_RequestId And sintBatchDateOptionalNight != 0
		) BDD
		On BCD.Nights = BDD.Nights
		And intRequestRunDetailId = p_RequestRunId  
		Where  BCD.Nights Is Null
	...
	INSERT INTO tmpReportUploadUpdate(intBatchId,nvcrBatchName,nvcrComment, IntCheckListID)	
	Select Distinct p_RequestId,v_nvcrBatchName, CONCAT('Missing Search: ' , Date_format(BD.sdtmBatchDateCrawl, 106) , ' Supplier:- ' , BD.nvcrDisplaySupplierName , '. Nights:- ' , Cast(BD.Nights As Varchar(50))), 239  
		 from LastResultQACheck BCD 
			   Right Join 
			(
				Select Distinct  BS.intSupplierId, BDD.sdtmBatchDateCrawl, MS.nvcrDisplaySupplierName, BDD.intBatchId, BND.Nights 
					FromBatchSupplier BS  inner join tbl_Competitor MS 
						On BS.intSupplierID =MS.intSupplierId
						-- Added by Bhushan Gaud on 16 Feb 2018 for CR received by Prachi(13 Feb 2018) Start
							And BS.intSupplierId in (Select Cast(Items As int) From dbo.Split(p_nvcrSupplierID, ','))
						-- Added by Bhushan Gaud on 16 Feb 2018 for CR received by Prachi(13 Feb 2018) End
					Full Outer JoinBatchDateDetail BDD   
					ON BS.intBatchId = BDD.intBatchId
					Full Outer Join 
					( 
						Select Distinct intBatchId, sdtmBatchDateCrawl, bitBatchDate1Night Nights FromBatchDateDetail    Where intBatchId = p_RequestId And bitBatchDate1Night = 1
						Union All
					...
					On BS.intBatchId = BND.intBatchId
					And BND.sdtmBatchDateCrawl = BDD.sdtmBatchDateCrawl
					Where BS.intBatchId = p_RequestId And BDD.intBatchId = p_RequestId
					And BND.intBatchId = p_RequestId
				) BD 
			On BCD.Company = BD.nvcrDisplaySupplierName  
			And Cast(BCD.Dates As Date)= Cast(BD.sdtmBatchDateCrawl As Date)
			And BCD.Nights = BD.Nights
			And intRequestRunDetailId = p_RequestRunId
 ...
 /*do not open
    ...
    Select  v_batchId,v_nvcrBatchName, 'Event type exist other than free and fixed', 192
	 from LastResultQACheck  BCD                             
    where intDiPBagDynamicId = p_RequestRunId 
    And ifnull(BCD.nvcrEventTypee,'') Not in ('Free', 'Fixed') Limit 1;
	 
	 
    
    INSERT INTO tmpReportUploadUpdate(intBatchId,nvcrBatchName,nvcrComment, IntCheckListID)
	Select  v_batchId,v_nvcrBatchName, 'Invalid Lead Time', 193
	 from LastResultQACheck  BCD;   inner join RequestRunDetail D 
	On BCD.intRequestRunDetailId = D.intRequestRunDetailId inner join tbl_RequestMaster MB 
	On D.intBatchId = MB.intBatchId inner join  `MstLeadTimeTemplate` MT 
	On MB.ncvrBatchName = MT.nvcrBatchName 
	And BCD.Dates = Case When MT.smdtCheckindate != '1900-01-01 00:00:00' Then cast(MT.smdtCheckindate As DATE) Else TIMESTAMPADD(Day, MT.intLTime,cast(MT.nvcrBookingDate As date)) End
	And BCD.Nights = MT.intNights
	And BCD.nvcrLeadTime != MT.nvcrLeadTime
	And MT.nvcrBatchName = v_nvcrBatchName
	WHERE  BCD.intRequestRunDetailId = p_RequestRunId Limit 1;
    
    
	INSERT INTO tmpReportUploadUpdate(intBatchId,nvcrBatchName,nvcrComment, IntCheckListID)
	Select Limit 1; v_batchId,v_nvcrBatchName, 'Invalid Event type', 194
	 from LastResultQACheck  BCD;   inner join RequestRunDetail D 
	On BCD.intRequestRunDetailId = D.intRequestRunDetailId inner join tbl_RequestMaster MB 
	On D.intBatchId = MB.intBatchId inner join  `MstLeadTimeTemplate` MT 
	On MB.ncvrBatchName = MT.nvcrBatchName 
	And BCD.nvcrLeadTime = MT.nvcrLeadTime
	And BCD.nvcrEventTypee != MT.nvcrEventTypee
	And MT.nvcrBatchName = v_nvcrBatchName
	WHERE  BCD.intRequestRunDetailId = p_RequestRunId 
    ...
    Having Count(Distinct nvcrEventTypee) > 1
    
    INSERT INTO tmpReportUploadUpdate(intBatchId,nvcrBatchName,nvcrComment, IntCheckListID)
    Select Limit 1; v_batchId,v_nvcrBatchName, CONCAT('Multiple Event Type exist' , Dates), 222
	 from LastResultQACheck  BCD;                             
    where intDiPBagDynamicId = p_RequestRunId 
    Group by Company, Dates
    Having Count(Distinct Nights) > 1
    And Count(Distinct nvcrEventTypee) =  1
 ...
 */
	...
	Select  p_RequestId,v_nvcrBatchName,'Room Code is null/blank ', 209  
 	...
 	INSERT INTO tmpReportUploadUpdate(intBatchId,nvcrBatchName,nvcrComment, IntCheckListID)
	SELECT Distinct v_batchId,v_nvcrBatchName, CONCAT('Single Room Code present :-' , A.Company + '-' + A.Dates + '-' + A.Nights +'-'+ RoomCode) , 210
	   From 
    ...
    INSERT INTO tmpReportUploadUpdate(intBatchId,nvcrBatchName,nvcrComment, IntCheckListID)
    Select Distinct v_batchId,v_nvcrBatchName,CONCAT('Matching hotel does not exist' , B.Dates + '-' + B.Company) , 211  From   
	(
	...
	INSERT INTO tmpReportUploadUpdate(intBatchId,nvcrBatchName,nvcrComment, IntStatus, IntCheckListID) 
		Select v_batchId,v_nvcrBatchName, 'account name against primary for TR batch is not ‘REVTRUK', 0 IntStatus, 247 
		from LastResultQACheck BCD Where intDiPBagDynamicId = p_RequestRunId
	...
	IF not exists (Select  Company  from LastResultQACheck BCD Where intDiPBagDynamicId = p_RequestRunId       
		...
		INSERT INTO tmpReportUploadUpdate(intBatchId,nvcrBatchName,nvcrComment, IntStatus, IntCheckListID) 
		  Select v_batchId,v_nvcrBatchName, 'account name against primary is ‘REVTRUK', 0 IntStatus, 249 
		   from LastResultQACheck BCD Where intDiPBagDynamicId = p_RequestRunId
		and nvcrAccountName = 'REVTRUK'
	...
	INSERT INTO DisplayData_UploadUpdate (`QA_Checks`,`Status`,`nvcrComment`,`intstatus`,`intbatchid`,`intRequestRunDetailid`,`nvcrSupplier`)
	Select 	ncvrCheckListName, `Status`, A.Comment, A.IntStatus,p_RequestId,p_RequestRunId,p_nvcrSupplierID  
	From 
	(
		...
		From MstCheckList Mst  WHere mst.intCheckList not in (Select IntCheckListID From tmpReportUploadUpdate )
